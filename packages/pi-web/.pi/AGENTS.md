...
工具 `description` 是 AI 理解工具用法的唯一入口，设计时注意：

- **只写方法论，不写具体内容**。教 AI 原则（怎么思考、怎么选择），不要给示例搜索词或硬编码参数
- **工具不做智能决策**。搜索词构造、目标网站选择、结果判断全交给 AI，工具只执行
- **每种工具的超时单独设**，在各自 `execute` 里的 `AbortSignal.timeout()` 控制
- 需要限制行为（如不读原文）写在工具描述即可，不要在代码层加硬限制
